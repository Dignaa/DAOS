...
export class GroupsController {
  ...
  async isUserInGroup(
    ...
    const isUserInGroup = await this.groupService.isUserInGroup(groupId, userId);
  ...
  async addUser(@Param('id') groupId: string, @Body('postId') postId: string, @Req() req: any) {
    ...
    if(group.userIds.find(user => user._id === new ObjectId(userId)) !== null) 
    {
    ...
    if(updatedGroup.userIds.find(user => user._id === new ObjectId(userId)) !== null) {
      await this.postService.remove(postId);
      return updatedGroup;
    }

    return new InternalServerErrorException("An error occured while trying to add a user into an enasmble. Please try again!");
  }
